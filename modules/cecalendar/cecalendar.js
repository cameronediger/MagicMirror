...
Module.register("cecalendar",{
	...
	getDom: function() {
		...
		wrapper.className = "align-left";
	
		console.log("cameron: " + events.length);	
		var weekday = moment().startOf("day");	//Start with Today
		for(var n=0; n < this.config.NumberOfDays; n++) {
			var dayWrapper = document.createElement("header");
			dayWrapper.innerHTML = moment(weekday).format("dddd");
			
			wrapper.appendChild(dayWrapper);
			
			var dayTableWrapper = document.createElement("table");
			dayTableWrapper.className = "small marginbottom";
			for(var e in events) {
				var event = events[e];
				
				console.log(moment(event.startDate, "x").format("YYYY MM DD") + ", " + moment(weekday).format("YYYY MM DD"));
				if(moment(event.startDate, "x").isSame(weekday, 'day')) {
					console.log("Im here");
					...
					var detailWrapper = document.createElement("td");
					detailWrapper.className  = "title bright";
					detailWrapper.innerHTML = this.titleTransform(event.title);
					eventWrapper.appendChild(detailWrapper);
					
					dayTableWrapper.appendChild(eventWrapper);
				}
				else if(moment(event.startDate, "x").isAfter(weekday, 'day')) {
					break;
				}
			}
			
			wrapper.appendChild(dayTableWrapper);
			weekday.add(1, 'days')
		}
